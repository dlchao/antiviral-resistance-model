/* driver
 *
 * creates a GlobalModel and outputs daily number of infecteds.
 * valgrind  --log-file=test -v global
 */
...
int main(int argc, char *argv[]) {
  ...
  g.city[114].addInfected(1000,0); // Hong Kong
  ...
  for (int t=params.nStartDay; t<runlength*365; t++) {
    ...
    for (unsigned int i=0; i<g.city.size(); i++) {
      CityModel &source = g.city[i];
      outfile << "," << source.getNumInfectedTotal();
    }
    outfile << endl;
    for (int j=0; j<nMaxStrains; j++) {
      outfile << g.getDay() << ",S" << j;
      for (unsigned int i=0; i<g.city.size(); i++) {
	CityModel &source = g.city[i];
	outfile << "," << (source.getNumInfected(j,0));
      }
      outfile << endl;
    }
    for (int j=0; j<nMaxStrains; j++) {
      outfile << g.getDay() << ",R" << j;
      for (unsigned int i=0; i<g.city.size(); i++) {
	CityModel &source = g.city[i];
	outfile << "," << (source.getNumInfected(j,1));
      }
      outfile << endl;
    }
  }
